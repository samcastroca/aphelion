...
**Las dos listas se construyen con políticas distintas, y ese es el cambio de
esta versión.** El reto evalúa los fragmentos con NDCG@10 y los documentos con
F1@3, en dos tablas clasificatorias independientes que después combina por
Conteo de Borda (§11). Nada obliga a que las dos salgan del mismo ranking, y
medido sobre el corpus completo no conviene que salgan:

    lista               política                       NDCG@10   F1@3
    fragmentos          BGE-M3 solo                     0,7329      —
    documentos          BGE-M3 + mE5-large, convexa        —     0,7547

Cada una toma la configuración que gana su métrica. Fusionar para los fragmentos
cuesta 0,0732 de NDCG@10 (p=0,0012); no fusionar para los documentos cuesta
0,0547 de F1@3. Usar una sola política obliga a pagar uno de los dos precios.

El flujo de recuperación es:

    codificar la consulta con el mismo encoder de la indexación
    buscar en cada índice, hasta la profundidad del pool
    fragmentos: ranking de BGE-M3 -> realce -> deduplicar -> diversificar -> top-10
    documentos: fusión convexa de los dos -> realce -> top2 pooling -> top-3
...
# --- Qué encoder decide cada lista -----------------------------------------
# El núcleo de esta versión. Las dos listas se evalúan por separado (§10.2) y se
# combinan por Borda (§11), así que cada una usa la política que gana su métrica.
#
# **Fragmentos: BGE-M3 solo.** Fusionar dos espacios vectoriales baja el NDCG@10
# de 0,7329 a 0,6597 con la convexa y a 0,6255 con RRF (p=0,0012 y p=0,0001,
# pareado sobre las 50 consultas del corpus completo). mE5-large aporta
# documentos que BGE-M3 no trae, pero ensucia el orden de los fragmentos.
#
# **Documentos: los dos, por fusión convexa.** Ahí la aportación de mE5 sí paga:
# el F1@3 sube de 0,7000 a 0,7547. La diferencia no alcanza significancia
# (p=0,1215) y se toma igual, porque el coste de equivocarse es nulo: la lista de
# fragmentos no cambia, así que el NDCG@10 es idéntico por construcción.
#
# Si solo hay un índice disponible, las dos listas caen sobre él y esta versión
# se comporta como la anterior.
FRAGMENT_ENCODERS = ("bge-m3",)
DOCUMENT_ENCODERS = ("bge-m3", "me5-large")

# Cómo se combinan los rankings para la lista de documentos. La convexa normaliza
# por min-max dentro de cada consulta y cada índice antes de sumar, que es lo que
# hace comparables dos escalas de coseno distintas —mE5 comprime todo hacia
# 0,75–0,92— sin tirar la magnitud como hace RRF al reducir todo a posiciones.
DOCUMENT_FUSION = "convex"
...
class QueryEncoder:
    ...
    def release(self) -> None:
        """Suelta el modelo y devuelve la memoria del acelerador.

        Hace falta porque esta versión usa dos encoders y **no caben los dos
        residentes** en una tarjeta modesta: BGE-M3 y mE5-large son ~560M
        parámetros cada uno y en fp32 ocupan ~2,2 GB, así que en 4 GB de VRAM el
        segundo revienta el proceso con una violación de segmento, no con una
        excepción que se pueda capturar. Como después de codificar las consultas
        solo se reordenan candidatos, ninguno hace falta más allá de su pasada.
        """
        self._model = None
        try:
            import gc
...
def fuse_convex(
    ...
    """Combinación convexa de similitudes normalizadas por consulta y por índice.

    La fusión con la que se construye la lista de documentos. Frente a RRF
    conserva la magnitud: RRF reduce todo a posiciones, de modo que un candidato
    que gana por 0,30 de coseno y otro que gana por 0,01 aportan lo mismo.
    Frente a sumar los cosenos crudos añade la normalización, que es lo que hace
    comparables dos escalas distintas — mE5-large comprime sus similitudes hacia
    0,75–0,92 y BGE-M3 se mueve más abajo, así que sumarlas sin normalizar deja
    que mE5 domine la suma por su escala y no por su criterio.

    La normalización es min-max **dentro de la consulta y del índice**: el mejor
    candidato de cada índice vale 1 y el peor 0. Si todos empatan, la
    normalización no está definida y se les da el mismo valor en vez de dividir
    por cero.
    ...
    for encoder, ranking in rankings.items():
        if not ranking:
            continue
        weight = (weights or {}).get(encoder, 1.0)
        scores = [s for _, s in ranking]
        top, bottom = max(scores), min(scores)
        spread = top - bottom

        for rank, (meta, score) in enumerate(ranking, start=1):
            normalized = 1.0 if spread <= 0 else (score - bottom) / spread
            chunk_id = meta["chunk_id"]
            candidate = pooled.get(chunk_id)
            if candidate is None:
                candidate = Candidate(
                ...
                pooled[chunk_id] = candidate
            candidate.score += weight * normalized
            candidate.ranks[encoder] = rank
...
def fuse(
    rankings: dict[str, list[tuple[dict, float]]], mode: str, k0: int = RRF_K0
) -> list[Candidate]:
    """Despacha a la fusión pedida. `convex` para documentos, `rrf` si no."""
    if mode == "convex":
        return fuse_convex(rankings)
    return fuse_rrf(rankings, k0)
...
class Retriever:
    def __init__(
        ...
        fragment_encoders: tuple[str, ...] = FRAGMENT_ENCODERS,
        document_encoders: tuple[str, ...] = DOCUMENT_ENCODERS,
        document_fusion: str = DOCUMENT_FUSION,
    ):
        if not indices:
        ...
        # Cada lista se queda con los encoders que pidió **y que existen**. Con
        # una base de un solo encoder las dos caen sobre él y el sistema se
        # comporta como la versión anterior, en vez de fallar por un índice que
        # el evaluador no recibió.
        self.fragment_encoders = tuple(e for e in fragment_encoders if e in indices)
        self.document_encoders = tuple(e for e in document_encoders if e in indices)
        if not self.fragment_encoders:
            self.fragment_encoders = tuple(indices)
        if not self.document_encoders:
            self.document_encoders = tuple(indices)
        self.document_fusion = document_fusion
    ...
    def search_all(self, queries: list[Query]) -> dict[str, dict[str, list]]:
        """Busca todas las consultas en todos los índices. query_id -> encoder -> ranking.

        **Un encoder a la vez, y se suelta al acabar su pasada.** Es lo que hace
        que esta versión funcione en una máquina modesta: con dos modelos
        residentes el proceso muere por falta de memoria, y con uno cada vez el
        pico es el mismo que el de una entrega de un solo encoder. El precio es
        recorrer las consultas dos veces, que son milisegundos.

        Codificar es lo caro; fusionar y reordenar después no cuesta nada, así
        que se busca una vez y las dos políticas trabajan sobre el mismo pool.
        """
        rankings: dict[str, dict[str, list]] = {q.query_id: {} for q in queries}

        for name, index in self.indices.items():
            encoder = self._encoder(name)
            vectors = encoder.encode([q.text for q in queries])
            for query, vector in zip(queries, vectors):
                hits = index.search(vector, self.candidates_per_index)
                ranking = [(index.metadata[pos], score) for pos, score in hits]
                rankings[query.query_id][name] = filter_by_relative_threshold(
                    ranking, self.threshold
                )
            encoder.release()
    ...
    def rank(
        self, query: Query, rankings: dict[str, list]
    ) -> tuple[list[str], list[Candidate]]:
        """Devuelve (top-3 documentos, candidatos a fragmento), por caminos distintos.

        No carga ningún modelo: solo reordena lo que `search_all` dejó en memoria.
        """
        # --- Fragmentos: el ranking de BGE-M3, sin fusionar --------------------
        ...
        # aquí: dos documentos distintos pueden compartir un texto y descartar
        # uno lo volvería inalcanzable.
        #
        ...
        para_fragmentos = fuse_rrf(
            {e: rankings[e] for e in self.fragment_encoders}, self.k0
        )
        para_fragmentos = apply_phenomenon_boost(
            para_fragmentos, query.phenomenon, self.boost
        )
        fragments = diversify(
            drop_duplicates(para_fragmentos), TOP_FRAGMENTS * 3, self.max_per_doc
        )

        # --- Documentos: los dos índices, por fusión convexa -------------------
        # Sobre el pool completo y no sobre la lista ya diversificada: el tope por
        # documento existe para repartir el riesgo entre las diez posiciones de
        # fragmento, y aplicarlo aquí descartaría evidencia de un documento que
        # justamente por concentrarla merece estar en el top-3.
        para_documentos = fuse(
            {e: rankings[e] for e in self.document_encoders},
            self.document_fusion,
            self.k0,
        )
        para_documentos = apply_phenomenon_boost(
            para_documentos, query.phenomenon, self.boost
        )
        documents = aggregate_to_documents(
            para_documentos, TOP_DOCUMENTS, self.aggregation
        )

        return documents, fragments

    def retrieve(self, query: Query) -> tuple[list[str], list[Candidate]]:
        """Una consulta suelta, de principio a fin. Cómodo para depurar.

        La generación de la entrega no pasa por aquí: usa `search_all` para no
        recargar los modelos cincuenta veces.
        """
        return self.rank(query, self.search_all([query])[query.query_id])
...
def main() -> int:
    ...
    ap.add_argument("--fragment-encoders", default=",".join(FRAGMENT_ENCODERS),
                    help="qué índices deciden el top-10 de fragmentos")
    ap.add_argument("--document-encoders", default=",".join(DOCUMENT_ENCODERS),
                    help="qué índices deciden el top-3 de documentos")
    ap.add_argument("--document-fusion", default=DOCUMENT_FUSION,
                    choices=("convex", "rrf"),
                    help="cómo se combinan los rankings para los documentos")
    args = ap.parse_args()
    ...
    retriever = Retriever(
        ...
        fragment_encoders=tuple(
            e.strip() for e in args.fragment_encoders.split(",") if e.strip()
        ),
        document_encoders=tuple(
            e.strip() for e in args.document_encoders.split(",") if e.strip()
        ),
        document_fusion=args.document_fusion,
    )
    # Que quede escrito en la salida con qué política se generó cada lista: si
    # alguien reproduce esto con otra base, la diferencia se ve aquí.
    print(f"fragmentos por: {'+'.join(retriever.fragment_encoders)}")
    print(f"documentos por: {'+'.join(retriever.document_encoders)}"
          f"  ({retriever.document_fusion}, {retriever.aggregation})")

    rankings = retriever.search_all(queries)
    records = []
    for query in queries:
        documents, fragments = retriever.rank(query, rankings[query.query_id])
