...
Uso:
    uv run python scripts/06_barrido.py
    uv run python scripts/06_barrido.py --rapido      # rejilla reducida
    uv run python scripts/06_barrido.py --aplicar     # escribe el ganador a config
"""
...
from aphelion import config, consultas as mod_consultas, metricas, recuperacion, salida
...
def main() -> int:
    ...
    if not args.ground_truth.exists():
        ...
        print("  1. uv run python scripts/05_pool_anotacion.py")
        print("  2. el equipo rellena la columna 'relevancia' en los CSV")
        print("  3. uv run python scripts/05_pool_anotacion.py --consolidar")
        print("  4. este script")
        return 1
