...
Uso:
    uv run python scripts/01b_ocr.py --muestra 2 --paginas 3   # inspección previa
    uv run python scripts/01b_ocr.py                           # el lote completo
"""
...
from aphelion import config, limpieza, ocr
...
def main() -> int:
    ...
    ap.add_argument(
        "--sin-cache", action="store_true", help="ignora datos/ocr.jsonl y reconoce todo"
    )
    args = ap.parse_args()

    lista = config.TRABAJO / "pendientes_ocr.txt"
    if not lista.exists():
        print(f"No existe {lista}. Ejecuta antes scripts/01_extraer.py")
        return 1
