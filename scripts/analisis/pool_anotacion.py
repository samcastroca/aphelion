...
Los CSV y el ground truth resultante viven en `data/`, versionados: son juicios
...
from aphelion.busqueda import consultas as mod_consultas, recuperacion
...
def main() -> int:
    ...
    ap.add_argument(
        "--consolidar",
        action="store_true",
        help="convierte los CSV ya anotados en data/ground_truth.jsonl",
    )
    args = ap.parse_args()
    ...
    if not disponibles:
        print("no hay índices construidos; ejecuta antes scripts/etapas/04_indexar.py")
        return 1
    ...
    print("Al terminar: uv run python scripts/analisis/pool_anotacion.py --consolidar")
    return 0
